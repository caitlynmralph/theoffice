...
with open('/Applications/MAMP/htdocs/theoffice/spreadsheets/percentdialogue_episode.csv', 'a') as csvfile:

    writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)

    counts = np.zeros([51,186], dtype=object)
    ...
    episodenames = ["Characters","Pilot", "DiversityDay", "HealthCare","TheAlliance","Basketball","HotGirl","TheDundies","SexualHarassment","OfficeOlympics","TheFire","Halloween","TheFight","TheClient","PerformanceReview","E-mailSurveillance","ChristmasParty",
                            ...
                            "ConflictResolution", "CasinoNight","GayWitchHunt", "TheConvention","TheCoup","GriefCounseling","Initiation","Diwali","BranchClosing","TheMerger","TheConvict","ABenihanaChristmas",
                            ...
                            "ProductRecall","Women'sAppreciation","BeachGames","TheJob","FunRun","DunderMifflinInfinity","LaunchParty","Money","LocalAd","BranchWars","SurvivorMan","TheDeposition","DinnerParty","ChairModel","NightOut",
                            "DidIStutter?","JobFair","GoodbyeToby","WeightLoss","BusinessEthics","BabyShower","CrimeAid","EmployeeTransfer","CustomerSurvey","BusinessTrip","FrameToby","TheSurplus","MoroccanChristmas",
                            ...
                            "MichaelScottPaperCompany","HeavyCompetition","Broke","CasualFriday","CafeDisco","CompanyPicnic","Gossip","TheMeeting","ThePromotion","Niagara","Mafia","TheLover","KoiPond","DoubleDate","Murder","ShareholderMeeting","Scott'sTots","SecretSanta","TheBanker",
                            "Sabre","ManagerandSalesman","TheDelivery","St.Patrick'sDay","NewLeads","HappyHour","Secretary'sDay","BodyLanguage","TheCover-Up","TheChump","Whistleblower","Nepotism","Counseling","Andy'sPlay","SexEd","TheSting","CostumeContest","Christening","ViewingParty","WUPHF.com","China","ClassyChristmas","Ultimatum",
                            ...
                            "DwightK.Schrute(Acting)Manager","SearchCommittee","TheList","TheIncentive","Lotto","GardenParty","Spooked","Doomsday","Pam'sReplacement","Gettysburg","Mrs.California","ChristmasWishes","Trivia","PoolParty",
                            ...
                            "FreeFamilyPortraitStudio","NewGuys","Roy'sWedding","AndysAncestry","WorkBus","HereComesTreble","TheBoat","TheWhale","TheTarget","DwightChristmas","Lice","SuitWarehouse",
    ...
    episodenames = np.reshape(episodenames,[1,len(episodenames)])

    counts = np.append(episodenames,counts,axis=0)
    ...
    for season in range(1,10):
        if season == 1:
            ...
            for episode in range(01,07):
                ...
                transcript = c[:]
                counts[1:2,episode:episode+1] = len(transcript)
                ...
                for line in range(0,len(transcript)):
                    for character in range(1,len(characters)):
                        if transcript[line:line+1,0:1][0][0].replace(" ","") == characters[character]:
                            counts[character+1:character+2,episode:episode+1] = int(counts[character+1:character+2,episode:episode+1]) + 1
        elif season == 2:
            ...
            for episode in range(1,23):
                if episode in range(0,10):
                    ...
                    transcript = c[:]
                    counts[1:2, episode+6:episode + 7] = len(transcript)
                    print len(transcript)
                    for line in range(0, len(transcript)):
                        for character in range(1, len(characters)):
                            if transcript[line:line + 1, 0:1][0][0].replace(" ", "") == characters[character]:
                                counts[character + 1:character + 2, episode+6:episode + 7] = int(
                                    counts[character + 1:character + 2, episode+6:episode + 7]) + 1
                else:
            ...
            for episode in range(1,24):
                if episode in range(0,10):
                    ...
                    c = np.genfromtxt(
                        ...
                        skiprows=0)
                    transcript = c[:]
                    counts[1:2, episode+28:episode + 29] = len(transcript)
                    print len(transcript)
                    for line in range(0, len(transcript)):
                        for character in range(1, len(characters)):
                            if transcript[line:line + 1, 0:1][0][0].replace(" ", "") == characters[character]:
                                counts[character + 1:character + 2, episode+28:episode + 29] = int(
                                    counts[character + 1:character + 2, episode+28:episode + 29]) + 1
                else:
            ...
            for episode in range(1,15):
                if episode in range(0,10):
                    ...
                    c = np.genfromtxt(
                        ...
                        skiprows=0)
                    transcript = c[:]
                    counts[1:2, episode+51:episode + 52] = len(transcript)
                    print len(transcript)
                    for line in range(0, len(transcript)):
                        for character in range(1, len(characters)):
                            if transcript[line:line + 1, 0:1][0][0].replace(" ", "") == characters[character]:
                                counts[character + 1:character + 2, episode+51:episode + 52] = int(
                                    counts[character + 1:character + 2, episode+51:episode + 52]) + 1
                else:
                    print '/Season%s/Season%s-Episode%s%s.csv' % (
                    str(season), str(season), str(episode), episodeNames[episode - 1])
                    c = np.genfromtxt(
            ...
            for episode in range(1,27):
                if episode in range(0,10):
                    ...
                    c = np.genfromtxt(
                        ...
                        skiprows=0)
                    transcript = c[:]
                    counts[1:2, episode+65:episode + 66] = len(transcript)
                    print len(transcript)
                    for line in range(0, len(transcript)):
                        for character in range(1, len(characters)):
                            if transcript[line:line + 1, 0:1][0][0].replace(" ", "") == characters[character]:
                                counts[character + 1:character + 2, episode+65:episode + 66] = int(
                                    counts[character + 1:character + 2, episode+65:episode + 66]) + 1
                else:
                    print '/Season%s/Season%s-Episode%s%s.csv' % (
                    str(season), str(season), str(episode), episodeNames[episode - 1])
                    c = np.genfromtxt(
            ...
            for episode in range(1,25):
                if episode in range(0,10):
                    ...
                    c = np.genfromtxt(
                        ...
                        skiprows=0)
                    transcript = c[:]
                    counts[1:2, episode+91:episode + 92] = len(transcript)
                    print len(transcript)
                    for line in range(0, len(transcript)):
                        for character in range(1, len(characters)):
                            if transcript[line:line + 1, 0:1][0][0].replace(" ", "") == characters[character]:
                                counts[character + 1:character + 2, episode+91:episode + 92] = int(
                                    counts[character + 1:character + 2, episode+91:episode + 92]) + 1
                else:
                    print '/Season%s/Season%s-Episode%s%s.csv' % (
                        str(season), str(season), str(episode), episodeNames[episode - 1])
                    c = np.genfromtxt(
            ...
            for episode in range(1,25):
                if episode in range(0,10):
                    ...
                    c = np.genfromtxt(
                        ...
                        skiprows=0)
                    transcript = c[:]
                    counts[1:2, episode+115:episode + 116] = len(transcript)
                    print len(transcript)
                    for line in range(0, len(transcript)):
                        for character in range(1, len(characters)):
                            if transcript[line:line + 1, 0:1][0][0].replace(" ", "") == characters[character]:
                                counts[character + 1:character + 2, episode+115:episode + 116] = int(
                                    counts[character + 1:character + 2, episode+115:episode + 116]) + 1
                else:
                    print '/Season%s/Season%s-Episode%s%s.csv' % (
                        str(season), str(season), str(episode), episodeNames[episode - 1])
                    c = np.genfromtxt(
            ...
            for episode in range(1,25):
                if episode in range(0,10):
                    ...
                    c = np.genfromtxt(
                        ...
                        skiprows=0)
                    transcript = c[:]
                    counts[1:2, episode+139:episode + 140] = len(transcript)
                    print len(transcript)
                    for line in range(0, len(transcript)):
                        for character in range(1, len(characters)):
                            if transcript[line:line + 1, 0:1][0][0].replace(" ", "") == characters[character]:
                                counts[character + 1:character + 2, episode+139:episode + 140] = int(
                                    counts[character + 1:character + 2, episode+139:episode + 140]) + 1
                else:
                    print '/Season%s/Season%s-Episode%s%s.csv' % (
                        str(season), str(season), str(episode), episodeNames[episode - 1])
                    c = np.genfromtxt(
            ...
            for episode in range(1,24):
                ...
                c = np.genfromtxt(
                    ...
                    delimiter="|", dtype=str, skiprows=0)
                transcript = c[:]
                counts[1:2,episode+163:episode+164] = len(transcript)
                print len(transcript)
                for line in range(0,len(transcript)):
                    for character in range(1,len(characters)):
                        if transcript[line:line+1,0:1][0][0].replace(" ","") == characters[character]:
                            counts[character+1:character+2,episode+163:episode+164] = int(counts[character+1:character+2,episode+163:episode+164]) + 1

    for e in range(1,187):
        "enter loop"
        for c in range(1,len(characters)):
            # print counts[0:1,e:e+1]
            counts[c + 1:c + 2, e:e + 1] = float(counts[c+1:c+2,e:e+1]) / float(counts[1:2,e:e+1])
